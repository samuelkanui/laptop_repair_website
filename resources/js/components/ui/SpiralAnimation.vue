...
const animationRef = ref<AnimationController | null>(null);
const dimensions = ref({ width: window.innerWidth, height: window.innerHeight });
...
onMounted(() => {
    ...
    // 使用全屏尺寸
    const size = Math.max(dimensions.value.width, dimensions.value.height);
    
    canvas.width = size * dpr;
    canvas.height = size * dpr;
    
    // 设置CSS尺寸
    canvas.style.width = `${dimensions.value.width}px`;
    canvas.style.height = `${dimensions.value.height}px`;
    
    // 缩放上下文以适应DPR
    ctx.scale(dpr, dpr);
    
    // 创建动画控制器
    animationRef.value = new AnimationController(canvas, ctx, dpr, size);
});
